...
export default function InvitarJuradoPage() {
  ...
  const addEmail = useCallback(() => {
    ...
    if (emailRegex.test(trimmedEmail)) {
      setEmailsList((prev) => (prev.includes(trimmedEmail) ? prev : [...prev, trimmedEmail]));
      setEmailInput("");
    }
  }, [emailInput]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-body relative">
      ...
      <div className="pb-[120px] lg:pb-12">
        ...
        >
          <div className="max-w-7xl mx-auto">
            ...
            <h1 className="text-3xl lg:text-4xl font-heading font-bold tracking-tight mb-2">Jurado</h1>
            <p className="text-blue-100 text-lg opacity-90">Gestión de Invitaciones al Jurado</p>
          </div>
        </header>

        <main
          className={`max-w-7xl mx-auto p-6 lg:p-10 space-y-12 transition-all duration-300 ${isPublicRole ? "lg:pl-10" : isCollapsed ? "lg:pl-28" : "lg:pl-80"}`}
        >
                ...
                <div className="space-y-4">
                  {pendientes.length === 0 ? (
                    ...
                    pendientes.map((j) => (
                      <div
                        key={j.id || j.email}
                        className="flex items-center justify-between group p-2 hover:bg-white rounded-2xl transition-all border border-transparent hover:border-gray-100"
                      >
                        <div className="flex items-center gap-3">
      ...
      {toast && (
        <div
          className={`fixed top-6 right-6 z-50 flex items-center gap-3 bg-white rounded-2xl shadow-xl p-4 border-l-4 ${
      ...
      {/* Modal confirmación eliminación: ESC => onClose/onCancel */}
      <ConfirmModal
        ...
        message={
          <>
            ...
            <strong className="font-semibold">{juradoToDelete?.nombreCompleto || juradoToDelete?.email}</strong>
            del panel de expertos. Esta acción no se puede deshacer.
          </>
        }
